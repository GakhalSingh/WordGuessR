...
class QuestionsViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        sendAnswerButton.isHidden = true
    ...
    func newQuestion() {
        ...
        if activatedMode == 0 {
            ...
            sendAnswerButton.isHidden = false
        }
        counterLabel.text = " "
    }
    ...
    func displayAnswer(){
        ...
        if  activatedMode == 0 || textField.text! == filteredQuestions[currentQuestionIndex].answer {
            correctAnswerCount += 1
            counterLabel.text = String(correctAnswerCount)
            sendAnswerButton.isEnabled = false
        }
    }
    @IBAction func returnAnswer(_ sender: Any) {
        textField.isEnabled = false
        if textField.text!.lowercased() == filteredQuestions[currentQuestionIndex].answer {
            correctAnswerCount += 1
            counterLabel.text = String(correctAnswerCount)
        } else {
